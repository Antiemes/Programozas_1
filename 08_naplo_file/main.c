...
int beker(struct diak_adat_t* diak, FILE* befile)
{
  char dummy;

  //printf("Nev: ");
  fgets(diak->nev, 30, befile);
  ...
  //printf("Matek: ");
  fscanf(befile, "%d", &(diak->matek));
  fscanf(befile, "%c", &dummy);
  
  //printf("Prog1: ");
  fscanf(befile, "%d", &(diak->prog1));
  fscanf(befile, "%c", &dummy);

  //printf("Elektronika: ");
  fscanf(befile, "%d", &(diak->elektronika));
  fscanf(befile, "%c", &dummy);
...
#define MAXDIAK 3
  ...
  while (!kilep)
  {
    if (beker(&diakok[diak_szamlalo], befile) < 0)
    {
      kilep = 1;
      diak_szamlalo--;
    }
  ...
  for (i=0; i<diak_szamlalo; i++)
  {
    for (j=i+1; j<diak_szamlalo; j++)
    {
      if (diakok[index[i]].prog1 > diakok[index[j]].prog1)
      {
        int temp;
        temp = index[i];
        index[i] = index[j];
